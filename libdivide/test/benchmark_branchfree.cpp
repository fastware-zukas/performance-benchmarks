...
template <typename T>
void benchmark(const char *name, tasks_t tasks, size_t max, size_t iters) {
  ...
  if (test_system && test_branchfull && branchfull.sum != sys.sum) {
    std::cerr << "Error: branchfull_divider<" << name
              << "> sum: " << branchfull.sum << ", but system sum: " << sys.sum
              << std::endl;
    std::exit(1);
  }

  if (test_system && test_branchfree && branchfree.sum != sys.sum) {
    std::cerr << "Error: branchfree_divider<" << name
              << "> sum: " << branchfree.sum << ", but system sum: " << sys.sum
              << std::endl;
    std::exit(1);
  }
  ...
  if (test_branchfull)
    std::cout << "branchfull: " << branchfull.duration << " seconds"
              << std::endl;
  if (test_branchfree)
    std::cout << "branchfree: " << branchfree.duration << " seconds"
              << std::endl;
...
int main(int argc, const char *argv[]) {
    ...
    if (tasks & TEST_U32) {
        ...
        benchmark<uint32_t>("uint32_t", tasks, max_divider, iters);
    }
    
    if (tasks & TEST_S32) {
        std::cout << "----- s32 -----" << std::endl;
        benchmark<int32_t>("int32_t", tasks, max_divider, iters);
    }
    
    if (tasks & TEST_U64) {
        std::cout << "----- u64 -----" << std::endl;
        benchmark<uint64_t>("uint64_t", tasks, max_divider, iters);
    }

    if (tasks & TEST_S64) {
        std::cout << "----- s64 -----" << std::endl;
        benchmark<int64_t>("int64_t", tasks, max_divider, iters);
    }
