...
public:
  bool operator()(const std::pair<std::string, int> &lhs,
    ...
    bool greater_amount = amountMap[lhs] > amountMap[rhs];
    bool equal_amount = amountMap[lhs] == amountMap[rhs];
...
static void record_vehicle_sale(std::string_view countryCode, int branchNumber,
                                int count) {
  amountMap[std::pair<std::string, int>(countryCode, branchNumber)] += count;
  ccbnSet.erase(std::pair<std::string, int>(countryCode, branchNumber));
  ccbnSet.insert(std::pair<std::string, int>(countryCode, branchNumber));
}
...
static std::vector<Result> get_top_branches(int top) {
  ...
  int size = std::min(static_cast<size_t>(top), ccbnSet.size());

  auto it = ccbnSet.begin();
  for (int i = 0; i < size; ++i) {
    const auto &ccbn = *it;
    result.push_back(Result{ccbn.first, ccbn.second, amountMap[ccbn]});
    it++;
  }
...
static void init() { volumes.resize(131071); }

static void record_vehicle_sale(std::string_view countryCode, int branchNumber,
                                int count) {

  hash_value hash = getHash(countryCode, branchNumber);
  auto current_volume = volumes[hash.value];
  uint32_t new_volume = current_volume + count;
  volumes[hash.value] = new_volume;

  SetItem item;
  item.hash = hash;
  item.volume = current_volume;

  auto set_node = ccbnSet.extract(item.key);
  item.volume = new_volume;
  if (!set_node.empty()) {
    set_node.value() = item.key;
    ccbnSet.insert(std::move(set_node));
  } else {
    ccbnSet.insert(item.key);
  }
}
